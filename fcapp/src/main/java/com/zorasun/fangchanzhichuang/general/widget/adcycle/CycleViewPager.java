...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
	...
	private List<AdListByPlay> infos;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		infos = new ArrayList<AdListByPlay>();
	...
	// public void setData(ArrayList<String> imageUrls, ImageCycleViewListener
	// listener) {
	// if (imageUrls.size() > 0) {
	//
	// List<ImageView> view1 = new ArrayList<ImageView>();
	// view1.clear();
	// if (infos != null) {
	// infos.clear();
	// }
	// for (int i = 0; i < imageUrls.size(); i++) {
	// if (imageUrls.get(i) != null && !imageUrls.get(i).equals("")) {
	// ADInfo info = new ADInfo();
	// info.setUrl(ApiConfig.getImageUrl(imageUrls.get(i)));
	// info.setContent("图片-->" + i);
	// infos.add(info);
	// }
	// }
	// // 将最后一个ImageView添加进来
	// view1.add(ViewFactory.getImageView(getActivity(),
	// infos.get(infos.size() == 0 ? 0 : infos.size() - 1).getUrl()));
	...
	private void setIndicator(int selectedPosition) {
		...
		for (int i = 0; i < indicators.length; i++) {
			indicators[i].setBackgroundResource(R.drawable.yuan2);
		}
		if (indicators.length > selectedPosition)
			indicators[selectedPosition].setBackgroundResource(R.drawable.yuan1);
	}
	...
	public static interface ImageCycleViewListener {
		 ...
		 * @param imageView
		 */
		public void onImageClick(AdListByPlay info, int postion, View imageView);
	}

	public void setData(List<AdListByPlay> adListByPlay, ImageCycleViewListener mAdCycleViewListener) {
		if (adListByPlay.size() > 0) {
			...
			for (int i = 0; i < adListByPlay.size(); i++) {
				if (adListByPlay.get(i) != null) {
					...
					AdListByPlay info1 = new AdListByPlay();
					info1.setBigImage(info.getBigImage());
					info1.setSign(info.getSign());
					info1.setBrokerId(info.getBrokerId());
					info1.setHouseResourceListId(info.getHouseResourceListId());
					info1.setNewHouseListId(info.getNewHouseListId());
					infos.add(info1);
				}
			}
			// 将最后一个ImageView添加进来
			view1.add(ViewFactory.getImageView(getActivity(),
					infos.get(infos.size() == 0 ? 0 : infos.size() - 1).getBigImage()));
			for (int i = 0; i < infos.size(); i++) {
				view1.add(ViewFactory.getImageView(getActivity(), infos.get(i).getBigImage()));
			}
			// 将第一个ImageView添加进来
			view1.add(ViewFactory.getImageView(getActivity(), infos.get(0).getBigImage()));

			setData(view1, mAdCycleViewListener, 0);
			if (infos.size() <= 1) {
				indicatorLayout.setVisibility(View.GONE);
			} else {
				indicatorLayout.setVisibility(View.VISIBLE);

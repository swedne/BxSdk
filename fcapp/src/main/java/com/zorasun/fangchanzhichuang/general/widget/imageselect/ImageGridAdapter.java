		...
		void bindData(final Image data)
		{
			...
			if (mItemSize > 0)
			{
				...
				Picasso.with(mContext).load(imageFile).placeholder(R.drawable.default_error)
						.error(R.drawable.default_error).resize(mItemSize, mItemSize).centerCrop().into(image);

				// 卡顿
				// AsyncImageLoader.setAsynAvatarImages(image, "sdcard://" + data.path);

				// // 使用Imageloader去加载图片
				// mImageLoader.loadImage(data.path, image);
			}
		}
	}

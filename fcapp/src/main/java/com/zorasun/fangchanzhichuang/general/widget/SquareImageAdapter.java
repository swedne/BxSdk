...
public class SquareImageAdapter extends BaseAdapter {
	...
	public SquareImageAdapter(Activity context, ArrayList<String> arrayList) {
	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		if (convertView == null) {
			...
			convertView = LayoutInflater.from(mContext).inflate(
					R.layout.item_square_image, null);
			viewHolder.iv = (ImageView) convertView.findViewById(R.id.iv);
			convertView.setTag(viewHolder);
		} else
			viewHolder = (ViewHolder) convertView.getTag();

		AsyncImageLoader.setAsynImages(viewHolder.iv,
				ApiConfig.getImageUrl(tempSelectBitmap.get(position)));
		return convertView;
	}
